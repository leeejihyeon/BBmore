				...
				<div class="col-md-12">
					<div class="content">
						...
						<ol class="breadcrumb">
							...
							<li class="active">freeService</li> 
						</ol>
					</div>
				</div>
			</div>
...
<section> 
	<div class="check"> 
		<i class="fa-solid fa-check fa-3x date-icon"></i><h3 class="p-date p-text"><b>수거 날짜를 선택해 주세요</b></h3> 
	</div>
	<div class="selecDate"><!-- 날짜선택부터 ~ 다음버튼까지 묶음 -->
		<form>
			...
			<div class="btn-daum">
				<button class="button-34 btnServiceCloth " role="button" type="button" onclick="pickupService();">다음</button> 
			</div>
		</form>
	</div>
</section>
...
<script>	
		...
		function pickupService() 
		{
			...
			$.ajax({
				type: "POST",
				url: "/order/freeServiceProc",
				data:
				{
					u_pickupDate:$('#datepicker1').val(),
					u_pickupTime:$("#selectTime").val()
				},
				datatype: "JSON",
				success:function(code)
				{
					if(code == 0)
					{
						swal("수거날짜 선택 중 오류가 발생했습니다.");
					}
					else if(code == 1)
					{	// 성공시
						location.href = "/order/chooseClothes";
					}
				},
